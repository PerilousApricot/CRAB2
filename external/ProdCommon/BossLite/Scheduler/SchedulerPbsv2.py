...
class SchedulerPbsv2 (SchedulerInterface) :
    ...
    def submitJob ( self, job, task=None, requirements=''):
        ...
        s.append('#PBS -e %s:%swrapper_%s' % (self.hostname, self.jobResDir, job['standardError']) )
        s.append('#PBS -o %s:%swrapper_%s' % (self.hostname, self.jobResDir, job['standardOutput']) )
        ...
        for file in job['outputFiles']:
            ...
            stageFile  = os.path.abspath( os.path.join( task['outputDirectory'],
                                                        file ) ) 
            if self.forceTransferFiles:
                s.append('#PBS -W stageout=%s@%s:%s/%s' % \
                    (targetFile,
                     self.hostname,
